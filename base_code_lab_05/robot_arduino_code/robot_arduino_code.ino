// Wifi setup (via ESP32 AT firmware on Serial3, pins 16=TX/17=RX)
#include <WiFiEspAT.h>
...
void setup() 
{
  ...
  // Initialize ESP32 AT modem on Serial3 (GIGA pins 16/17)
  Serial3.begin(115200);
  delay(2000); // Wait for ESP32 to boot
  Serial3.println("AT+CWMODE=1"); // Set station mode explicitly
  delay(500);  // Wait for mode change to take effect
  WiFi.init(Serial3);
  ...
  // Scan for networks to verify ESP32 can see the router
  ...
  // Clear any cached WiFi credentials from ESP32 flash
  WiFi.disconnect();
  ...
  while (WiFi.localIP() == IPAddress(0, 0, 0, 0)) {
    ...
    if (++attempts > 30) {
      Serial.println("Failed to get IP after 30s. Halting.");
      while (true);
    }
  }
  Serial.println();
  ...
  // Start lidar motor first, then serial, then bind driver
  pinMode(RPLidarMotorPin, OUTPUT);
  analogWrite(RPLidarMotorPin, 255);
  delay(2000); // wait for motor to reach speed

  // Bind the RPLIDAR driver to the arduino hardware serial
  // Must call Serial2.begin() here — RPLidar::begin() no longer does it
  // (calling begin/end on Mbed OS UART hangs if not properly initialized)
...
void forward(int speed)
{
  ...
  analogWrite(RightSpeedPin, speed * 0.495 );
}
...
void lidar_update() {
  if (IS_OK(lidar.waitPoint())) {
    ...
    if (IS_OK(lidar.getDeviceInfo(info, 100))) {
       ...
       analogWrite(RPLidarMotorPin, 255);
       // NOTE: removed delay(1000) — blocking here starves UDP receive and
       // can cause the WiFiEspAT AT-command channel to time out, dropping WiFi.
    }
  }
}
