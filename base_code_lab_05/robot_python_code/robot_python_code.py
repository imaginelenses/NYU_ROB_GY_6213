...
class UDPCommunication:
    def __init__(self, arduinoIP, localIP, arduinoPort, localPort, bufferSize):
        ...
        self.UDPServerSocket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEPORT, 1)  # macOS needs this to rebind quickly
        self.UDPServerSocket.setblocking(False)  # non-blocking: recvfrom returns immediately
        # Bind to all interfaces ('') rather than the specific local IP.
        # On macOS, binding to a specific IP that momentarily disappears (WiFi
        # reconnect, DHCP renew) invalidates the socket and causes sendto to
        # fail with EHOSTUNREACH even after the network comes back.
        self.UDPServerSocket.bind(('', localPort))
    ...
    # Explicitly close the socket (call before discarding this object)
    def close(self):
        try:
...
class CameraSensor:
    ...
    def __init__(self, camera_id):
        ...
        self.cap = None  # Don't open camera here
